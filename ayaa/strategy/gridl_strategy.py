...
class GridTradingStrategy:
    ...
    def execute_strategy2(self, current_date, current_price, current_data):
        ...
        if self.last_macd < 0 and current_data['macd_diff'] >= 0:
            if self.last_stash_quantity > 0:
                quantity += self.last_stash_quantity
                self.last_stash_quantity = 0
            else:
                self.last_stash_quantity = 0
        elif self.last_macd > 0 and current_data['macd_diff'] <= 0:
            ...
            self.last_stash_quantity += quantity
            quantity = 0
    ...
    def backtest(self, symbol, data):
        ...
        for idx, row in data.iterrows():
            ...
            self.execute_strategy(row['date'], row['close'], row)
